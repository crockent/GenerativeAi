...
def naiveReconstruction(points, normals, X, Y, Z):
    ...
    # tree.query for k=1 mighr return a 2D array instead of an index so we must check 
    # the condition and take the first column 
    if idx.ndim > 1: 
        idx = idx[:, 0]
    nearest_pts = points[idx]        
    nearest_normals = normals[idx]   
    vectors = Q - nearest_pts  
    signed_distances = np.sum(vectors * nearest_normals, axis=1)
