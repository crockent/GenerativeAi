...
def train(dataset, model, optimizer, args):
    ...
    for i in range(num_batch):
        ...
        sigma = 0.1
        pred_sdf_clamped = torch.clamp(pred_sdf_tensor,-sigma,sigma)
        real_sdf_clamed = torch.clamp(sdf_tensor,-sigma,sigma)
        loss = torch.abs(pred_sdf_clamped-real_sdf_clamed).mean()
...
def main(args):
    ...
    model = Decoder(dims=[512, 512, 512, 512, 512, 512, 512, 1],
